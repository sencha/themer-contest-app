...
folder are mapped to the application's root namespace, 'ThemerContestApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ThemerContestApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ThemerContestApp' in the same way
as `"ThemerContestApp/sass/src"`.
